...
ifeq ($(PLATFORM),WINDOWS)
    duke3d_rsrc := $(duke3d_root)/rsrc/awol
endif
duke3d_obj := $(obj)/awol
...
ifeq (11,$(HAVE_GTK2)$(STARTUP_WINDOW))
    ...
    duke3d_game_gen_objs += game_creditsBMP.c
...
$$($1_obj)/%_creditsBMP.c: $$($1_rsrc)/credits.bmp | $$($1_obj)
	echo "#include \"gtkpixdata_shim.h\"" > $$@
	gdk-pixbuf-csource --extern --struct --raw --name=startcreditsBMP_pixdata $$^ | sed 's/load_inc//' >> $$@
