...
static const char *defaultconfilename                = "awol.con";
#ifndef EDUKE32_STANDALONE
static const char *defaultgamegrp[GAMECOUNT]         = { "awol.grp", "NAM.GRP", "NAPALM.GRP", "WW2GI.GRP", "awol.grp" };
static const char *defaultdeffilename[GAMECOUNT]     = { "awol.def", "nam.def", "napalm.def", "ww2gi.def", "awol.def" };
static const char *defaultgameconfilename[GAMECOUNT] = { "awol.con", "NAM.CON", "NAPALM.CON", "WW2GI.CON", "awol.con" };
#endif
...
const char *G_DefaultGrpFile(void)
{
    ...
    if (DUKE)
        ...
        return defaultgamegrp[GAME_AWOL];

    return defaultgamegrp[4];
#else
    return "(none)";
#endif
}
const char *G_DefaultDefFile(void)
{
    ...
    else if (NAM)
    {
        ...
        return defaultdeffilename[GAME_AWOL];
...
const char *G_DefaultConFile(void)
{
    ...
    else if (AWOL && testkopen(defaultgameconfilename[GAME_AWOL],0))
        return defaultgameconfilename[GAME_AWOL];
#endif
...
void G_ScanGroups(void)
{
    ...
    g_selectedGrp = foundgrps; // g_selectedGrp = NULL;
...
{
    char buf[BMAX_PATH];
	/* Disabled these search paths in AWOL
...
void G_AddSearchPaths(void)
{
    ...
    // Ion Fury - GOG.com
	Bsnprintf(buf, sizeof(buf), cwd, homepath); // Bsnprintf(buf, sizeof(buf), "%s/GOG Games/ION Fury", homepath); Change for AWOL
...
void G_LoadGroupsInDir(const char *dirname)
{
    ...
    // static const char *extensions[] = { "awol.grp" };
    char buf[BMAX_PATH];
