...
{
    if (FURY) return;
    // Original: Menu_DrawCursorCommon(x, y, z, VM_OnEventWithReturn(EVENT_MENUCURSORLEFT, -1, myconnectindex, SPINNINGNUKEICON+((timer120()>>3)%7)));
	// First Edit: Menu_DrawCursorCommon(x, y, 32768, VM_OnEventWithReturn(EVENT_MENUCURSORLEFT, -1, myconnectindex, SPINNINGNUKEICON+((timer120()>>3)%7)));
	Menu_DrawCursorCommon(x, y, 32768, VM_OnEventWithReturn(EVENT_MENUCURSORLEFT, -1, myconnectindex, SPINNINGNUKEICON));
}
static void Menu_DrawCursorRight(int32_t x, int32_t y, int32_t z)
{
    if (FURY) return;
    // Original: Menu_DrawCursorCommon(x, y, z, VM_OnEventWithReturn(EVENT_MENUCURSORRIGHT, -1, myconnectindex, SPINNINGNUKEICON+6-((6+(timer120()>>3))%7)));
	// First Edit: Menu_DrawCursorCommon(x, y, 32768, VM_OnEventWithReturn(EVENT_MENUCURSORRIGHT, -1, myconnectindex, SPINNINGNUKEICON+6-((6+(timer120()>>3))%7)));
	Menu_DrawCursorCommon(x, y, 32768, VM_OnEventWithReturn(EVENT_MENUCURSORRIGHT, -1, myconnectindex, SPINNINGNUKEICON));
}
...
static void Menu_DrawCursorText(int32_t x, int32_t y, int32_t h, int32_t ydim_upper = 0, int32_t ydim_lower = ydim-1)
{
    ...
    Menu_DrawCursorTextTile(x, y, h, SPINNINGNUKEICON, siz, ydim_upper, ydim_lower);
    // Original: Menu_DrawCursorTextTile(x, y, h, SPINNINGNUKEICON+((timer120()>>3)%7), siz, ydim_upper, ydim_lower);
	// First Edit: Menu_DrawCursorTextTile(x, y, 32768, SPINNINGNUKEICON+((timer120()>>3)%7), siz, ydim_upper, ydim_lower);
}
...
static char const *MEOSN_CrosshairOffOn[] = { "Off", "On", "Crosshair Only", };
// static MenuOptionSet_t MEOS_CrosshairOffOn = MAKE_MENUOPTIONSET( MEOSN_CrosshairOffOn, NULL, 0x3 );
static int32_t MEOSV_CrosshairOffOn[] = { 0, 1, 2, };
static MenuOptionSet_t MEOS_CrosshairOffOn = MAKE_MENUOPTIONSET( MEOSN_CrosshairOffOn, MEOSV_CrosshairOffOn, 0x2 );

static char const *MEOSN_ADSMODE_ToggleHold[] = { "Toggle", "Hold", }; // awol adsmode menu test
static MenuOptionSet_t MEOS_ADSMODE_ToggleHold = MAKE_MENUOPTIONSET( MEOSN_ADSMODE_ToggleHold, NULL, 0x3 ); // awol adsmode menu test

// static char const *MEOSN_SubtitlesOffOn[] = { "Off", "On", }; // awol subtitles menu test
static MenuOptionSet_t MEOS_SubtitlesOffOn = MAKE_MENUOPTIONSET( MEOSN_OffOn, NULL, 0x3 ); // awol subtitles menu test

static char const *MEOSN_WeatherOffLowNormal[] = { "Off", "Low", "Normal", }; // weather menu option
static int32_t MEOSV_WeatherOffLowNormal[] = { 0, 1, 2, }; // weather menu option
static MenuOptionSet_t MEOS_WeatherOffLowNormal = MAKE_MENUOPTIONSET( MEOSN_WeatherOffLowNormal, MEOSV_WeatherOffLowNormal, 0x2 ); // weather menu option

// static char const *MEOSN_ParticlesOffOn[] = { "Off", "On", }; // particles menu option
static MenuOptionSet_t MEOS_ParticlesOffOn = MAKE_MENUOPTIONSET( MEOSN_OffOn, NULL, 0x3 ); // particles menu option

static char const *MEOSN_TextSpeedOffSlowNormalFast[] = { "Off", "Slow", "Normal", "Fast", }; // Text speed menu option
static int32_t MEOSV_TextSpeedOffSlowNormalFast[] = { 0, 1, 2, 3, }; // Text speed menu option
static MenuOptionSet_t MEOS_TextSpeedOffSlowNormalFast = MAKE_MENUOPTIONSET( MEOSN_TextSpeedOffSlowNormalFast, MEOSV_TextSpeedOffSlowNormalFast, 0x3 ); // Text speed menu option

// static char const *MEOSN_PerfMdlOffOn[] = { "Off", "On", }; // particles menu option
static MenuOptionSet_t MEOS_PerfModelOffOn = MAKE_MENUOPTIONSET( MEOSN_OffOn, NULL, 0x3 ); // particles menu option

/* START REMARKING NEXT LINE, ALTHOUGH IT MIGHT NOT NEED TO MOVE AT ALL
static char const *MEOSN_text_speedOffSlowNormalFast[] = { "Off", "Slow", "Normal", "Fast", }; // Text speed menu option
static int32_t MEOSV_text_speedOffSlowNormalFast[] = { 0, 1, 2, 3, }; // Text speed menu option
static MenuOptionSet_t MEOS_text_speedOffSlowNormalFast = MAKE_MENUOPTIONSET( MEOSN_text_speedOffSlowNormalFast, MEOSV_text_speedOffSlowNormalFast, 0x3 ); // Text speed menu option
// END OF REMARKING AREA */
...
/*
static MenuOption_t MEO_GAMESETUP_ADSMODE = MAKE_MENUOPTION( &MF_Redfont, &MEOSN_ADSMODE_ToggleHold, &ud.adsmode); // awol adsmode menu test
static MenuEntry_t ME_GAMESETUP_ADSMODE = MAKE_MENUENTRY( "ADS MODE:", &MF_Redfont, &MEF_BigOptionsRt, &MEOSN_ADSMODE_ToggleHold, Option ); // awol adsmode menu test
*/
static MenuOption_t MEO_SCREENSETUP_ADSMODE = MAKE_MENUOPTION(&MF_Redfont, &MEOS_ADSMODE_ToggleHold, &ud.adsmode); // awol adsmode menu test
static MenuEntry_t ME_SCREENSETUP_ADSMODE = MAKE_MENUENTRY( "ADS MODE:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_SCREENSETUP_ADSMODE, Option ); // awol adsmode menu test
...
static MenuOption_t MEO_GAMESETUP_WEATHER = MAKE_MENUOPTION( &MF_Redfont, &MEOS_WeatherOffLowNormal, &ud.weather ); // weather menu option
static MenuEntry_t ME_GAMESETUP_WEATHER = MAKE_MENUENTRY( "Weather:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_GAMESETUP_WEATHER, Option );
static MenuOption_t MEO_GAMESETUP_PARTICLES = MAKE_MENUOPTION(&MF_Redfont, &MEOS_ParticlesOffOn, &ud.particles); // particles menu option
static MenuEntry_t ME_GAMESETUP_PARTICLES = MAKE_MENUENTRY( "Particle Effects:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_GAMESETUP_PARTICLES, Option ); // particles menu option
static MenuOption_t MEO_SCREENSETUP_TEXTSPEED = MAKE_MENUOPTION(&MF_Redfont, &MEOS_TextSpeedOffSlowNormalFast, &ud.text_speed); // Text speed menu option
static MenuEntry_t ME_SCREENSETUP_TEXTSPEED = MAKE_MENUENTRY( "Text Speed:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_SCREENSETUP_TEXTSPEED, Option ); // Text speed menu option
static MenuOption_t MEO_GAMESETUP_PERFMDL = MAKE_MENUOPTION(&MF_Redfont, &MEOS_PerfModelOffOn, &ud.perf_mdl); // particles menu option
static MenuEntry_t ME_GAMESETUP_PERFMDL = MAKE_MENUENTRY( "3D Models:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_GAMESETUP_PERFMDL, Option ); // particles menu option
...
static MenuEntry_t *MEL_GAMESETUP[] = {
    // &ME_ADULTMODE,
    &ME_GAMESETUP_WEATHER, // weather menu option
    &ME_GAMESETUP_PARTICLES, // particles menu option
	&ME_SCREENSETUP_TEXTSPEED, // Text speed menu option
	&ME_GAMESETUP_PERFMDL, // Model performance mode
#if defined STARTUP_SETUP_WINDOW && !defined EDUKE32_RETAIL_MENU
...
static char const *MEOSN_VIDEOSETUP_RENDERER[] = { "OpenGL", };
static int32_t MEOSV_VIDEOSETUP_RENDERER[] = { REND_POLYMOST, };
#endif
...
static MenuEntry_t ME_VIDEOSETUP_RENDERER = MAKE_MENUENTRY( NULL, &MF_Redfont, &MEF_Null, &MEO_Space2, Spacer );
#endif
...
static MenuEntry_t ME_DISPLAYSETUP_COLORCORR = MAKE_MENUENTRY( "Colour Correction", &MF_Redfont, &MEF_BigOptionsRt, &MEO_DISPLAYSETUP_COLORCORR, Link );
...
static MenuOptionSet_t MEOS_DISPLAYSETUP_ASPECTRATIO = MAKE_MENUOPTIONSET( MEOSN_DISPLAYSETUP_ASPECTRATIO, MEOSV_DISPLAYSETUP_ASPECTRATIO, 0x3 );
...
// static MenuOption_t MEO_SCREENSETUP_CROSSHAIR = MAKE_MENUOPTION(&MF_Redfont, MEOS_CrosshairOffOn, &ud.crosshair);
static MenuOption_t MEO_SCREENSETUP_CROSSHAIR = MAKE_MENUOPTION( &MF_Redfont, &MEOS_CrosshairOffOn, &ud.crosshair );
...
static MenuEntry_t ME_SCREENSETUP_SCREENSIZE = MAKE_MENUENTRY( NULL, &MF_Redfont, &MEF_Null, &MEO_Space2, Spacer );
...
static MenuOption_t MEO_SCREENSETUP_SUBTITLES = MAKE_MENUOPTION(&MF_Redfont, &MEOS_OffOn, &ud.subtitles); // awol subtitles menu test
static MenuEntry_t ME_SCREENSETUP_SUBTITLES = MAKE_MENUENTRY( "Subtitles:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_SCREENSETUP_SUBTITLES, Option ); // awol subtitles menu test
/* START REMARKING NEXT LINE
static MenuOption_t MEO_SCREENSETUP_text_speed = MAKE_MENUOPTION(&MF_Redfont, &MEOS_text_speedOffSlowNormalFast, &ud.text_speed); // Text speed menu option
static MenuEntry_t ME_SCREENSETUP_text_speed = MAKE_MENUENTRY( "Text Speed:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_SCREENSETUP_text_speed, Option ); // Text speed menu option
// END OF REMARKING AREA */
...
static MenuEntry_t ME_SCREENSETUP_SBARSIZE = MAKE_MENUENTRY( NULL, &MF_Redfont, &MEF_Null, &MEO_Space2, Spacer ); // scale slider just below "STATUS BAR:" 
...
static MenuEntry_t *MEL_CONTROLS[] = {
    ...
    // &ME_GAMESETUP_ALWAYS_RUN, // awol adsmode menu test
	&ME_SCREENSETUP_ADSMODE, // awol adsmode menu test
...
static MenuRangeInt32_t MEO_JOYSTICK_WEIGHTED_AIMING = MAKE_MENURANGE(&ud.config.JoystickAimWeight , &MF_Redfont, 0, 8, 0, 9, 0 );
...
static MenuRangeInt32_t MEO_JOYSTICK_VIEW_CENTERING = MAKE_MENURANGE(&ud.config.JoystickViewCentering , &MF_Redfont, 0, 8, 0, 5, 0 );
...
static MenuOption_t MEO_JOYSTICKAXIS_ANALOG = MAKE_MENUOPTION( &MF_Redfont, &MEOS_JOYSTICKAXIS_ANALOG, NULL );
static MenuEntry_t ME_JOYSTICKAXIS_ANALOG = MAKE_MENUENTRY( "Input", &MF_Redfont, &MEF_BigSliders, &MEO_JOYSTICKAXIS_ANALOG, Option );
static MenuRangeInt32_t MEO_JOYSTICKAXIS_SCALE = MAKE_MENURANGE( NULL, &MF_Redfont, 0, 131072, 131072, 101, 2 | EnforceIntervals );
...
static MenuRangeInt32_t MEO_JOYSTICKAXIS_DEAD = MAKE_MENURANGE( NULL, &MF_Redfont, 0, 4000, 0, 21, EnforceIntervals );
...
static MenuRangeInt32_t MEO_JOYSTICKAXIS_SATU = MAKE_MENURANGE( NULL, &MF_Redfont, 6000, 10000, 0, 21, EnforceIntervals );
...
static MenuOption_t MEO_JOYSTICKAXIS_DIGITALNEGATIVE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_Gamefuncs, NULL );
static MenuEntry_t ME_JOYSTICKAXIS_DIGITALNEGATIVE = MAKE_MENUENTRY( "Digital -", &MF_Redfont, &MEF_BigSliders, &MEO_JOYSTICKAXIS_DIGITALNEGATIVE, Option );
// static MenuOption_t MEO_JOYSTICKAXIS_DIGITALPOSITIVE = MAKE_MENUOPTION( &MF_Minifont, &MEOS_Gamefuncs, NULL );
static MenuOption_t MEO_JOYSTICKAXIS_DIGITALPOSITIVE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_Gamefuncs, NULL );
static MenuEntry_t ME_JOYSTICKAXIS_DIGITALPOSITIVE = MAKE_MENUENTRY( "Digital +", &MF_Redfont, &MEF_BigSliders, &MEO_JOYSTICKAXIS_DIGITALPOSITIVE, Option );
...
// static MenuEntry_t ME_RENDERERSETUP_HIGHTILE = MAKE_MENUENTRY( "True colour textures:", &MF_Bluefont, &MEF_SmallOptions, &MEO_RENDERERSETUP_HIGHTILE, Option );
static MenuEntry_t ME_RENDERERSETUP_HIGHTILE = MAKE_MENUENTRY( "True colour textures:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_RENDERERSETUP_HIGHTILE, Option );
...
static MenuEntry_t ME_RENDERERSETUP_PRECACHE = MAKE_MENUENTRY( "Pre-load map textures:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_RENDERERSETUP_PRECACHE, Option );
# ifndef EDUKE32_GLES
...
static MenuEntry_t ME_RENDERERSETUP_TEXCACHE = MAKE_MENUENTRY( "On-disk texture cache:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_RENDERERSETUP_TEXCACHE, Option );
# endif
# ifdef USE_GLEXT
...
static MenuEntry_t ME_POLYMER_LIGHTS = MAKE_MENUENTRY("Dynamic lights:", &MF_Redfont, &MEF_SmallOptions, &MEO_POLYMER_LIGHTS, Option);

static MenuRangeInt32_t MEO_POLYMER_LIGHTPASSES = MAKE_MENURANGE(&r_pr_maxlightpasses, &MF_Redfont, 1, 10, 1, 10, 1);
static MenuEntry_t ME_POLYMER_LIGHTPASSES = MAKE_MENUENTRY("Lights per surface:", &MF_Redfont, &MEF_SmallOptions, &MEO_POLYMER_LIGHTPASSES, RangeInt32);

static MenuOption_t MEO_POLYMER_SHADOWS = MAKE_MENUOPTION(&MF_Redfont, &MEOS_OffOn, &pr_shadows);
static MenuEntry_t ME_POLYMER_SHADOWS = MAKE_MENUENTRY("Dynamic shadows:", &MF_Redfont, &MEF_SmallOptions, &MEO_POLYMER_SHADOWS, Option);

static MenuRangeInt32_t MEO_POLYMER_SHADOWCOUNT = MAKE_MENURANGE(&pr_shadowcount, &MF_Redfont, 1, 10, 1, 10, 1);
static MenuEntry_t ME_POLYMER_SHADOWCOUNT = MAKE_MENUENTRY("Shadows per surface:", &MF_Redfont, &MEF_SmallOptions, &MEO_POLYMER_SHADOWCOUNT, RangeInt32);
...
#ifdef EDUKE32_ANDROID_MENU
static MenuRangeFloat_t MEO_COLCORR_GAMMA = MAKE_MENURANGE( &g_videoGamma, &MF_Redfont, 1.f, 2.5f, 0.f, 39, 1 );
#else
static MenuRangeFloat_t MEO_COLCORR_GAMMA = MAKE_MENURANGE( &g_videoGamma, &MF_Redfont, 0.3f, 4.f, 0.f, 75, 1 );
#endif
static MenuEntry_t ME_COLCORR_GAMMA = MAKE_MENUENTRY( "Gamma:", &MF_Redfont, &MEF_ColorCorrect, &MEO_COLCORR_GAMMA, RangeFloat );
static MenuRangeFloat_t MEO_COLCORR_CONTRAST = MAKE_MENURANGE( &g_videoContrast, &MF_Redfont, 0.1f, 2.7f, 0.f, 53, 1 );
...
static MenuRangeFloat_t MEO_COLCORR_BRIGHTNESS = MAKE_MENURANGE( &g_videoBrightness, &MF_Redfont, -0.8f, 0.8f, 0.f, 33, 1 );
...
#endif
#ifndef EDUKE32_RETAIL_MENU
static MenuRangeFloat_t MEO_COLCORR_AMBIENT = MAKE_MENURANGE( &r_ambientlight, &MF_Redfont, MINVIS, 4.f, 0.f, 32, 1 );
...
static MenuEntry_t *MEL_SCREENSETUP[] = {
...
#endif
    // &ME_SCREENSETUP_SCREENSIZE, // Prevent the status bar...
    // &ME_SCREENSETUP_SBARSIZE, // ... and scale options from appearing in HUD SETUP when disabled
    ...
    //&ME_SCREENSETUP_SUBTITLES, // awol subtitles menu test
	/* START REMARKING NEXT LINE
    &ME_SCREENSETUP_text_speed, // Text speed menu option
	// END OF REMARKING AREA */
};
...
static MenuString_t MEO_PLAYER_NAME = MAKE_MENUSTRING( szPlayerName, &MF_Redfont, MAXPLAYERNAME, 0 );
static MenuEntry_t ME_PLAYER_NAME = MAKE_MENUENTRY( "Name", &MF_Redfont, &MEF_PlayerNarrow, &MEO_PLAYER_NAME, String );
...
static MenuOption_t MEO_PLAYER_COLOR = MAKE_MENUOPTION( &MF_Redfont, &MEOS_PLAYER_COLOR, &ud.color );
static MenuEntry_t ME_PLAYER_COLOR = MAKE_MENUENTRY( "Colour", &MF_Redfont, &MEF_PlayerNarrow, &MEO_PLAYER_COLOR, Option );
...
static MenuOption_t MEO_PLAYER_TEAM = MAKE_MENUOPTION( &MF_Redfont, &MEOS_PLAYER_TEAM, &ud.team );
static MenuEntry_t ME_PLAYER_TEAM = MAKE_MENUENTRY( "Team", &MF_Redfont, &MEF_PlayerNarrow, &MEO_PLAYER_TEAM, Option );
#ifndef EDUKE32_RETAIL_MENU
static MenuLink_t MEO_PLAYER_MACROS = { MENU_MACROS, MA_Advance, };
static MenuEntry_t ME_PLAYER_MACROS = MAKE_MENUENTRY( "Multiplayer macros", &MF_Redfont, &MEF_SmallOptions, &MEO_PLAYER_MACROS, Link );
#endif
...
static MenuOption_t MEO_NETOPTIONS_GAMETYPE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_NETOPTIONS_GAMETYPE, &ud.m_coop );
...
static MenuOption_t MEO_NETOPTIONS_EPISODE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_NETOPTIONS_EPISODE, &NetEpisode );
...
static MenuOption_t MEO_NETOPTIONS_LEVEL = MAKE_MENUOPTION( &MF_Redfont, NULL, &ud.m_level_number );
...
static MenuOption_t MEO_NETOPTIONS_MONSTERS = MAKE_MENUOPTION( &MF_Redfont, &MEOS_NETOPTIONS_MONSTERS, NULL );
static MenuEntry_t ME_NETOPTIONS_MONSTERS = MAKE_MENUENTRY( "Monsters", &MF_Redfont, &MEF_NetSetup, &MEO_NETOPTIONS_MONSTERS, Option );
static MenuOption_t MEO_NETOPTIONS_MARKERS = MAKE_MENUOPTION( &MF_Redfont, &MEOS_OffOn, &ud.m_marker );
static MenuEntry_t ME_NETOPTIONS_MARKERS = MAKE_MENUENTRY( "Markers", &MF_Redfont, &MEF_NetSetup, &MEO_NETOPTIONS_MARKERS, Option );
static MenuOption_t MEO_NETOPTIONS_MAPEXITS = MAKE_MENUOPTION( &MF_Redfont, &MEOS_OnOff, &ud.m_noexits );
...
static MenuOption_t MEO_NETOPTIONS_FRFIRE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_OffOn, &ud.m_ffire );
...
static MenuString_t MEO_NETJOIN_SERVER = MAKE_MENUSTRING( MenuServer, &MF_Redfont, BMAX_PATH, 0 );
...
static MenuString_t MEO_NETJOIN_PORT = MAKE_MENUSTRING( MenuPort, &MF_Redfont, MAXPORTSTRINGLENGTH, INPUT_NUMERIC );
...
static MenuMenu_t M_RENDERERSETUP_POLYMOST = MAKE_MENUMENU( "Polymost Setup", &MMF_BigOptions, MEL_RENDERERSETUP_POLYMOST );
# ifdef POLYMER
static MenuMenu_t M_RENDERERSETUP_POLYMER = MAKE_MENUMENU("Polymer Setup", &MMF_SmallOptions, MEL_RENDERERSETUP_POLYMER );
# endif
#endif
static MenuMenu_t M_COLCORR = MAKE_MENUMENU( "Colour Correction", &MMF_ColorCorrect, MEL_COLCORR );
...
static MenuPanel_t M_CREDITS = { NoTitle, MENU_CREDITS5, MA_Return, MENU_CREDITS4, MA_Advance, };
...
static MenuPanel_t M_CREDITS4 = { "About EDUKE32", MENU_CREDITS, MA_Return, MENU_CREDITS5, MA_Advance, };
...
static void Menu_PreDraw(MenuID_t cm, MenuEntry_t *entry, const vec2_t origin)
{
    ...
    case MENU_COLCORRRESETVERIFY:
        videoFadeToBlack(1);
        Menu_DrawVerifyPrompt(origin.x, origin.y, "Reset colour correction to defaults?");
        break;
...
// #endif
    ...
    if (entry == &ME_GAMESETUP_WEAPSWITCH_PICKUP) // Changed to "if" since the preceeding if statement has been disabled
    {
        ud.weaponswitch &= ~(1|4);
        switch (newOption)
        {
