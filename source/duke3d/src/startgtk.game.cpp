...
static struct
{
    ...
    GtkWidget *credits_title;
    GtkWidget *credits_spacer;
    GtkWidget *credits_grid;
    GtkWidget *creditsBMP;
} stwidgets;
...
static GdkPixbuf *load_creditsBMP(void)
{
    // return gdk_pixbuf_from_pixdata((GdkPixdata const *)&startcreditsBMP_pixdata, FALSE, NULL);
    return gdk_pixbuf_from_pixdata((GdkPixdata const *)&startcreditsBMP_pixdata, FALSE, NULL); // This line is givimg me grief
}
...
// GAMES TAB MOVED TO HERE
	...
	// creditsBMP
    {
        // GdkPixbuf *pixbuf = load_creditsBMP();
        GdkPixbuf *pixbuf = load_creditsBMP();
        // stwidgets.creditsBMP = gtk_image_new_from_pixbuf(pixbuf);
        stwidgets.creditsBMP = gtk_image_new_from_pixbuf(pixbuf);
        g_object_unref((gpointer)pixbuf);
    }
    // gtk_box_pack_start(GTK_BOX(stwidgets.hlayout), stwidgets.creditsBMP, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(stwidgets.gamevlayout), stwidgets.creditsBMP, FALSE, FALSE, 0);
    // gtk_misc_set_alignment(GTK_MISC(stwidgets.creditsBMP), 0.5, 0);
    gtk_misc_set_alignment(GTK_MISC(stwidgets.creditsBMP), 0.5, 0);
...
*/
    // Game tab
    stwidgets.gametab = gtk_label_new("Credits");
    gtk_notebook_set_tab_label(GTK_NOTEBOOK(stwidgets.tabs), gtk_notebook_get_nth_page(GTK_NOTEBOOK(stwidgets.tabs), 2), stwidgets.gametab);
