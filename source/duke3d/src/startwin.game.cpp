...
static void PopulateForm(int32_t pgs)
{
    ...
    if (pgs & POPULATE_VIDEO)
    {
        ...
        for (int i=0; i<validmodecnt; i++)
        {
			...
			*/
			// Populate the list of video modes with only OpenGL list items
			if (validmode[i].bpp == 32)
			{
				Bsprintf(buf, "%dx%d %s", validmode[i].xdim, validmode[i].ydim, validmode[i].bpp == 32 ? "" : "");
				int const j = ComboBox_AddString(hwnd, buf);
				(void)ComboBox_SetItemData(hwnd, j, i);
				if (i == mode)(void)ComboBox_SetCurSel(hwnd, j);
			}
...
{
    static HBITMAP hbmp = NULL, hbmpCredits = NULL;
    ...
    case WM_INITDIALOG:
    {
        ...
        hbmpCredits = LoadBitmap((HINSTANCE)win_gethinstance(), MAKEINTRESOURCE(RSRC_CREDITS));

		HWND hwnd = GetDlgItem(hwndDlg, WIN_STARTWIN_CREDITS);
		SendMessage(hwnd, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hbmpCredits);
		
		hbmp = LoadBitmap((HINSTANCE)win_gethinstance(), MAKEINTRESOURCE(RSRC_BMP));

        hwnd = GetDlgItem(hwndDlg, WIN_STARTWIN_BITMAP);  // Remove "HWND" if adding 2nd image (first one is currently disabled.)
        ...
        // Add tabs to the tab control
        {
            ...
            static char textCredits[] = TEXT("Credits"); // New line added for use by AWOL game launcher
            ...
            // Start of new lines added for use by AWOL game launcher
            tab.mask = TCIF_TEXT;
            tab.pszText = textCredits;
            SendMessage(hwnd, TCM_INSERTITEM, (WPARAM)TAB_CREDITS, (LPARAM)&tab);
            // End of new lines added for use by AWOL game launcher
            ...
            // Start of new lines added for use by AWOL game launcher
			pages[TAB_CREDITS] = CreateDialog((HINSTANCE)win_gethinstance(), MAKEINTRESOURCE(WIN_STARTWINPAGE_CREDITS), hwndDlg, NULL);
			SetWindowPos(pages[TAB_CREDITS], hwnd, r.left, r.top, r.right, r.bottom, SWP_HIDEWINDOW);
    ...
    case WM_DESTROY:
        ...
        // Start of new lines added for use by AWOL game launcher
        if (pages[TAB_CREDITS])
        {
            DestroyWindow(pages[TAB_CREDITS]);
            pages[TAB_CREDITS] = NULL;
        }
        // End of new lines added for use by AWOL game launcher
        startupdlg = NULL;
        return TRUE;
